...
public class CellSelector extends ScrollArea {
    ...
    protected void onClick( PointerEvent event ) {
        Gdx.app.log("InputDebug", "Click detected!");
        // FORCE THE CALL HERE to verify connection from click to attack in realtime mode
        if (RealtimeInput.isEnabled()) {
            if (Dungeon.hero != null) {
                Dungeon.hero.performRealtimeAttack();
            }
            return; // Stop other click logic
        }
        if (dragging) {
    ...
    private float zoom( float value ) {
        ...
        // In realtime mode allow selection even if hero.ready is false, but still block when UI is covering
        if (listener != null && cell != -1 && !GameScene.interfaceBlockingHero()
                && (RealtimeInput.isEnabled() || (enabled && Dungeon.hero.ready))) {
    ...
    protected void onDrag( PointerEvent event ) {
        // Disable manual camera dragging while in realtime mode
        if (RealtimeInput.isEnabled()) {
            return;
        }
    ...
    private boolean delayingForRelease = false;
        ...
        // Realtime tile-hold pacing (deprecated by smooth movement)
    private float realtimeMoveDelay = 0f;
    private static final float REALTIME_MOVE_INTERVAL = 0.09f;
    ...
    private Signal.Listener<KeyEvent> keyListener = new Signal.Listener<KeyEvent>() {
        ...
        public boolean onSignal(KeyEvent event) {
            GameAction action = KeyBindings.getActionForKey( event );
                        ...
                        // Realtime mode: capture WASD/arrow keys as continuous state, and block turn-based movement handling
            if (RealtimeInput.isEnabled()) {
                boolean handledRealtime = false;
                if (action == SPDAction.W) {
                    RealtimeInput.moveLeft = event.pressed;
                    handledRealtime = true;
                } else if (action == SPDAction.E) {
                    RealtimeInput.moveRight = event.pressed;
                    handledRealtime = true;
                } else if (action == SPDAction.N) {
                    RealtimeInput.moveUp = event.pressed;
                    handledRealtime = true;
                ...
                // Also consume diagonals and wait/rest so turn-based logic doesn't trigger while in realtime
                        ...
                        || action == SPDAction.REST) {
                    return true;
                }


                if (handledRealtime) {
                    return true; // consume so turn-based movement does not process
                }
            }
    ...
    public void update() {
        super.update();
				...
				// Realtime mode: handle continuous keyboard movement and block turn-based processing
		if (RealtimeInput.isEnabled()) {
						leftStickAction = SPDAction.NONE;
			heldAction1 = heldAction2 = heldAction3 = SPDAction.NONE;

			// Safety: if no physical directional keys are pressed, clear any stuck realtime movement
			boolean physLeft  = Gdx.input.isKeyPressed(Input.Keys.A) || Gdx.input.isKeyPressed(Input.Keys.LEFT);
			boolean physRight = Gdx.input.isKeyPressed(Input.Keys.D) || Gdx.input.isKeyPressed(Input.Keys.RIGHT);
			boolean physUp    = Gdx.input.isKeyPressed(Input.Keys.W) || Gdx.input.isKeyPressed(Input.Keys.UP);
			boolean physDown  = Gdx.input.isKeyPressed(Input.Keys.S) || Gdx.input.isKeyPressed(Input.Keys.DOWN);
			if (!(physLeft || physRight || physUp || physDown)) {
				RealtimeInput.resetMovement();
			}

						// In realtime, hero movement is handled in Hero.updateRealtime; just return to block turn-based processing
			return;
		}
    ...
    private int lastCellMoved = 0;
        ...
        // Deprecated in realtime smooth movement
    private boolean moveByDelta(int dx, int dy){
        ...
        if (GameScene.cancelCellSelector()){
            return false;
                ...
                dx = (int) GameMath.gate(-1, dx, 1);
        dy = (int) GameMath.gate(-1, dy, 1);

                int cell = Dungeon.hero.pos + dx + dy * Dungeon.level.width();
        if (cell != Dungeon.hero.pos && cell != lastCellMoved){
            // Only allow movement into passable/avoidable tiles without occupants
            if ((Dungeon.level.passable[cell] || Dungeon.level.avoid[cell])
                    && Actor.findChar(cell) == null
                    // avoid moving into open pits in realtime to prevent falling via scheduler
                    && !(Dungeon.level.pit[cell] && !Dungeon.level.solid[cell])){
                lastCellMoved = cell;
                Dungeon.hero.sprite.move(Dungeon.hero.pos, cell);
